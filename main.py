...
class ImprovedTableFootballScheduler:
    ...
    def generate_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        """
        print(f"Generating optimal schedule for {num_rounds} rounds... (this may take a while)", file=sys.stderr)
    ...
    def _generate_optimal_5_6_player_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        print("Using sitting-rotation-first optimization for perfect balance...", file=sys.stderr)
    ...
    def _generate_sitting_enforced_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        print(f"Enforcing perfect sitting rotation for {num_rounds} rounds...", file=sys.stderr)
        ...
        print(f"Sitting-enforced schedule complete: {len(schedule)} rounds", file=sys.stderr)
        return schedule
    ...
    def _generate_fair_sitting_rotation(self, sitting_count: int) -> List[List[str]]:
        ...
        print(f"Generated {len(sitting_rotation)} sitting combinations for {self.num_players} players", file=sys.stderr)
        print(f"Pattern repeats every {rotation_length} rounds for fair distribution", file=sys.stderr)
    ...
    def _generate_segmented_optimal_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        print(f"Using segmented approach for {num_rounds} rounds...", file=sys.stderr)
        ...
        while current_round <= num_rounds:
            ...
            print(f"Optimizing rounds {current_round}-{current_round + this_segment_size - 1}...", file=sys.stderr)
        ...
        print(f"Segmented optimization complete. Total rounds: {len(full_schedule)}", file=sys.stderr)
    ...
    def _generate_limited_backtrack_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        print(f"Using limited backtracking for {num_rounds} rounds...", file=sys.stderr)
        ...
        for strategy in strategies:
            ...
            if score > best_score:
                ...
                print(f"New best with {strategy}: {score:.2f}", file=sys.stderr)
    ...
    def _generate_optimal_4_player_schedule(self, num_rounds: int) -> List[Tuple[int, str, str, str, str, str]]:
        ...
        print("Using intelligent transition-based strategy for 4-player solution...", file=sys.stderr)
        ...
        for round_num in range(2, num_rounds + 1):
            ...
            if best_encounter:
                ...
                print(f"Warning: could not find good encounter for round {round_num}", file=sys.stderr)
                break

        print(f"Intelligent 4-player schedule complete: {len(schedule)} rounds", file=sys.stderr)
        return schedule
    ...
    def _score_4_player_transition(self, encounter: Tuple, prev_encounter: Tuple,
        ...
        # HIGH PRIORITY: Global team distribution balance
        # Calculate current team usage and heavily penalize overused teams
        team1 = t1_def + t1_att
        team2 = t2_def + t2_att

        current_team_counts = self._calculate_current_team_counts(schedule)
        team1_count = current_team_counts.get(team1, 0)
        team2_count = current_team_counts.get(team2, 0)

        # Calculate expected frequency at this point
        total_rounds_so_far = len(schedule)
        if total_rounds_so_far > 0:
            # Each team should appear roughly (total_rounds * 2) / 12 times
            # (multiply by 2 because each round has 2 teams)
            expected_frequency = (total_rounds_so_far * 2) / 12

            # Heavy penalty for teams that are significantly overused
            team1_overuse = max(0, team1_count - expected_frequency)
            team2_overuse = max(0, team2_count - expected_frequency)

            if team1_overuse > 0.5:  # More than half a round over expected
                score -= team1_overuse * 1000  # Very heavy penalty
            if team2_overuse > 0.5:
                score -= team2_overuse * 1000

            # Bonus for underused teams
            team1_underuse = max(0, expected_frequency - team1_count)
            team2_underuse = max(0, expected_frequency - team2_count)

            if team1_underuse > 0.5:
                score += team1_underuse * 500  # Good bonus for balancing
            if team2_underuse > 0.5:
                score += team2_underuse * 500

        # Additional penalty for overused encounter patterns globally
        usage_count = encounter_usage.get(encounter_key, 0)
        if usage_count > 0:
            score -= usage_count * 300  # Increased penalty
    ...
    def _calculate_current_team_counts(self, schedule: List) -> Dict[str, int]:
        """Calculate how many times each team has appeared in the schedule so far"""
        team_counts = {}
